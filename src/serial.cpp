...
{// currently only support baud rate 115200
	printf("running connect_serial function. The UART PORT thats gonna be checked is %d \n",port);
	const char *arm_path[] = { "/dev/ttyUSB0", "/dev/ttyUSB1", "/dev/ttyTHS2" };
	
	g_sdk_uart_fd = open("/dev/ttyTHS2", O_RDWR | O_NOCTTY);
	//printf("paei gia to arm_path[port]=%c \n", arm_path[port]);
	//cout<<arm_path[port]<<endl;	
